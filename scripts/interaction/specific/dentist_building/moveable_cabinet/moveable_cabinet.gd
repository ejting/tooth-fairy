...
var moved_cabinet_pos := Vector3(-580.118, 765.022, 44.371)
var original_cabinet_pos := Vector3(-521.105, 765.022, 44.371)
...
func blackout_finished(dm_ref : DialogueManager):
	# Play sound
	if(dialogue_index == 0):
		to_move.position = moved_cabinet_pos
	elif(dialogue_index == 1):
		to_move.position = original_cabinet_pos
